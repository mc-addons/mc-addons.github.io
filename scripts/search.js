...
async function checkForFile(dir){
    result = await getDataFromFile(dir)
...
async function readData() {
    let searchedWords = location.search.substring(8)

    let found = await checkForFile("search/" + searchedWords + ".search")

    console.log("Searching for: " + searchedWords)
    console.log("Exact search is: " + "search/" + searchedWords + ".search")
    console.log(found)
    ...
    if(!found){
        ...
        searchedAddons = await getDataFromFile("search/" + searchedWords + ".search")

        searchedAddons = searchedAddons.split("|").filter(e =>  e)

        console.log(searchedAddons)

        for(let i = 0; i < searchedAddons.length; i++){
            addons.push(await getDataFromFile("addons/" + searchedAddons[i] + ".addon"))
        }
